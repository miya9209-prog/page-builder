...
def to_noun(line: str) -> str:
    ...
    s = s.replace('분께도', '분')
    s = s.replace('분께', '분')
    s = re.sub(r'(추천드립니다|추천합니다|권해드립니다|권합니다|권해드려요|추천드려요|좋습니다|잘 어울립니다|이상적입니다|알맞습니다|적합합니다|만족을 드립니다|만족을 드릴 선택입니다|입니다)$', '', s).strip()
...
def normalize_recommend_line(line: str) -> str:
    s = re.sub(r'\s+', ' ', (line or '').strip())
    s = s.strip('"“”\' ')
    s = s.rstrip('. ')
    replacements = {
    ...
    for a,b in replacements.items():
        s = s.replace(a,b)
    s = re.sub(r'(적극\s*)?(권해드립니다|권합니다|추천드립니다|추천합니다|추천드려요|권해드려요|좋습니다|잘 어울립니다|이상적입니다|알맞습니다|적합합니다|만족하실 거예요|만족하실 선택입니다|만족하실 제품입니다|만족하실 제품)$', '', s).strip()
    s = re.sub(r'(을|를|이|가|은|는)\s*찾으신다면$', r'\1 찾으시는 분', s)
    s = re.sub(r'(을|를|이|가|은|는)\s*원하신다면$', r'\1 원하시는 분', s)
    s = re.sub(r'원하신다면$', '원하시는 분', s)
    s = re.sub(r'찾으신다면$', '찾으시는 분', s)
    s = re.sub(r'원하시는$', '원하시는 분', s)
    s = re.sub(r'찾으시는$', '찾으시는 분', s)
    s = re.sub(r'선호하시는$', '선호하시는 분', s)
    s = re.sub(r'중요하게 생각하신다면$', '중요하게 생각하시는 분', s)
    s = re.sub(r'원하는$', '원하는 분', s)
    s = re.sub(r'찾는$', '찾는 분', s)
    s = re.sub(r'싶은$', '싶은 분', s)
    s = s.replace('분 분', '분')
    s = re.sub(r'\s+', ' ', s).strip(' .')
    if not s.endswith('분'):
        s += ' 분'
...
def tidy_wrapped_html(text: str) -> str:
    s = text.replace('<br>\n.', '.')
    s = s.replace('<br>\n,', ',')
    s = s.replace(' .', '.')
    s = s.replace(' ,', ',')
...
5) recommend_lines는 반드시 "~분" 형태의 대상 정의 문장으로만 작성한다. "분께", "고객님께", "추천합니다", "권해드립니다", "적합합니다" 금지.
6) 문장은 너무 짧게 끊지 말고, 한 줄 단위로 보기 좋게 정리한다.
...
- recommend_lines: 반드시 아래 예시처럼 대상 정의 문장 4줄. 예시: "오피스룩으로 깔끔하게 입고 싶은 분", "체형을 자연스럽게 커버하고 싶은 분", "다양한 하의와 쉽게 코디하고 싶은 분", "구김 없이 편하게 입고 싶은 분". 절대 "추천합니다/권해드립니다/적합합니다/잘 어울립니다/고객님께" 같은 문장형 금지. 모든 문장은 반드시 "분"으로 끝나야 한다.
...
def render_text_source(structured: Dict[str, Any]) -> str:
    rec_lines = ''.join(["▪ " + tidy_wrapped_html(wrap_text_source(normalize_recommend_line(x))) + "<br>\n" for x in structured['recommend_lines']])
    ...
    for idx, faq in enumerate(structured['faqs']):
        ...
        if idx < len(structured['faqs']) - 1:
            faq_lines.append("<br>\n")
    shopping_lines = ''.join(["▪ " + tidy_wrapped_html(wrap_text_source(clean_line(x))) + "<br>\n" for x in structured['shopping_lines']])
...
def render_subsc_html(data: Dict[str, str], structured: Dict[str, Any]) -> str:
    ...
    def join_lines(lines: List[str]) -> str:
        return ''.join([f"{tidy_wrapped_html(wrap_md(clean_line(x)))}<br>\n" for x in lines if clean_line(x)])
