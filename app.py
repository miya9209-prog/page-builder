...
def sentence_to_point_phrase(text: str) -> str:
    ...
    replacements = [
        ('조화롭게 혼합되어 부드럽고 편안한 착용감을 선사합니다', '부드럽고 편안한 착용감'),
        ('균형 있게 혼합되어 부드럽고 쾌적한 착용감을 선사합니다', '균형 잡힌 혼방 소재감'),
        ('은은한 광택이 더해져 세련된 무드를 연출하며, 구김이 적어 관리가 간편합니다', '은은한 광택과 구김 적은 관리 편의성'),
        ('은은한 광택이 더해져 세련된 무드를 연출하며, 구김이 적어 관리가 편리합니다', '은은한 광택과 구김 적은 관리 편의성'),
        ('가볍고 부담 없는 두께감으로 여리한 실루엣이 자연스럽게 표현됩니다', '가볍고 부담 없는 두께감과 여리한 실루엣'),
        ('적당한 두께감으로 여리한 실루엣이 자연스럽게 표현됩니다', '적당한 두께감으로 자연스럽게 살아나는 여리한 실루엣'),
    ]
    for a, b in replacements:
        s = s.replace(a, b)
    s = s.replace('입니다', '').replace('합니다', '').replace('됩니다', '')
    s = re.sub(r'\s+', ' ', s).strip(' .')
    if not s:
        return ''
    if not s.endswith('.'):
...
def point_phrase_from_keyword(phrase: str, mode: str = "detail") -> str:
    p = normalize_phrase(phrase)
    if not p:
        return ''
    mapping = [
        (r'탈부착|타이', '탈부착 가능한 타이 디테일로 다양한 스타일 연출.'),
        (r'브이넥', '브이넥 라인으로 얼굴선이 더 슬림해 보이는 효과.'),
        (r'볼륨\s*소매|소매', '볼륨감 있는 소매로 여성스러운 실루엣.'),
        (r'앞\s*절개|절개라인', '앞 절개 라인으로 상체가 슬림해 보이는 인상.'),
        (r'구김', '구김 걱정이 적어 데일리로 부담 없는 활용성.'),
        (r'체형\s*커버|군살\s*커버', '군살을 자연스럽게 커버하는 세련된 핏.'),
        (r'오피스|하객|데일리', '오피스룩, 하객룩, 데일리룩까지 폭넓은 활용성.'),
        (r'핏', '체형을 자연스럽게 정리해 주는 안정감 있는 핏.'),
    ]
    for pattern, out in mapping:
...
def normalize_detail_or_appeal_lines(block_body: str, input_text: str, fallback_lines: list[str], mode: str = "detail") -> list[str]:
    phrases = [normalize_phrase(x) for x in re.split(r'<br\s*/?>|\n+|/|,', block_body) if normalize_phrase(x)]
    if len(phrases) <= 1:
        phrases = split_phrases(input_text) or phrases
    lines = []
    for x in phrases[:6]:
        line = point_phrase_from_keyword(x, mode=mode)
...
def wrap_answer_lines(answer: str) -> str:
    ...
    if len(answer) <= 40:
        return answer + '<br>'
    split_at = max(answer.rfind(' ', 0, 34), answer.rfind(' ', 0, 40))
    if split_at == -1:
        return answer + '<br>'
...
def assemble_final_output(raw_result: str, source_block: str, data: Dict[str, str]):
    ...
    sec4 = format_point_block('4. (디테일컷)', normalize_detail_or_appeal_lines(get_block_body(sec4, '4. (디테일컷)'), data.get('detail_tip') or '', ['디테일이 살아 있는 세련된 포인트.', '작은 차이가 전체 분위기를 더 정돈해 주는 완성도.'], mode='detail'))
    sec5 = format_point_block('5. (핵심어필 포인트)', normalize_detail_or_appeal_lines(get_block_body(sec5, '5. (핵심어필 포인트)'), data.get('appeal_points') or data.get('fit') or '', ['체형을 자연스럽게 커버하는 실용적인 매력.', '매일 손이 가는 편안한 활용성.'], mode='appeal'))
...
if st.session_state.generated_result:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.download_button("TXT 다운로드", data=st.session_state.generated_result, file_name=f"{st.session_state.generated_filename}.txt", mime="text/plain", use_container_width=True)
    with c2:
        st.download_button("HWP 다운로드", data=st.session_state.generated_docx, file_name=f"{st.session_state.generated_filename}.hwp", mime="application/x-hwp", use_container_width=True)
