...
7) 추천 블록(recommend_lines)은 반드시 아래 형식으로만 작성한다.
   - 오피스룩으로 깔끔하게 입고 싶은 분
   - 체형을 자연스럽게 커버하고 싶은 분
   - 다양한 하의와 쉽게 코디하고 싶은 분
   - 구김 없이 편하게 입고 싶은 분
8) 추천 블록에서 절대 금지:
   - 추천합니다 / 권해드립니다 / 적합합니다 / 잘 어울립니다 / 좋습니다
   - 분께 / 고객님께 / 고객님
   - 문장 끝에 광고성 어미 사용
9) 추천 블록의 모든 문장은 반드시 "분"으로 끝나야 한다.
...
def render_text_source(data: Dict[str, str], structured: Dict[str, Any]) -> str:
    recommend_lines = build_recommend_lines(data, structured)
    rec_lines = ''.join([f"▪ {x}<br>\n" for x in recommend_lines])
    review_lines = ''.join([f'"{sanitize_review_text(x)}"<br>\n' for x in structured['review_lines']])
    faq_lines = []
    for idx, faq in enumerate(structured['faqs']):
        q = tidy_wrapped_html(wrap_text_source(strip_q_prefix(faq['q'])))
        a = tidy_wrapped_html(wrap_text_source(strip_a_prefix(faq['a'])))
        faq_lines.append(f"Q. {q}<br>\n")
        faq_lines.append(f"A. {a}<br>\n")
        if idx < len(structured['faqs']) - 1:
            faq_lines.append("<br>\n")
    shopping_lines = ''.join([f"▪ {tidy_wrapped_html(wrap_text_source(clean_line(x)))}<br>\n" for x in structured['shopping_lines']])
...
def assemble_final_output(data: Dict[str, str], structured: Dict[str, Any]) -> str:
    ...
    text_source = render_text_source(data, structured)
...
def build_recommend_lines(data: Dict[str, str], structured: Dict[str, Any]) -> List[str]:
    product = clean_line(data.get("display_name", ""))
    fit = clean_line(data.get("fit", ""))
    appeal = clean_line(data.get("appeal_points", ""))
    details = clean_line(data.get("detail_tip", ""))
    material_desc = clean_line(data.get("material_desc_raw", ""))

    lines: List[str] = []
    if "오피스" in appeal or "오피스" in details or "오피스" in material_desc:
        lines.append("오피스룩으로 깔끔하게 입고 싶은 분")
    else:
        lines.append("단정한 자리에도 자연스럽게 입고 싶은 분")

    if any(k in (fit + " " + appeal + " " + details) for k in ["체형", "군살", "커버", "여유"]):
        lines.append("체형을 자연스럽게 커버하고 싶은 분")
    else:
        lines.append("여유 있는 실루엣을 편하게 입고 싶은 분")

    if "셔츠" in product or "블라우스" in product:
        lines.append("다양한 하의와 쉽게 코디하고 싶은 분")
    else:
        lines.append("한 벌로 다양한 무드를 연출하고 싶은 분")

    if any(k in material_desc for k in ["구김", "관리", "내구", "손세탁", "드라이"]):
        lines.append("구김 없이 편하게 입고 싶은 분")
    else:
        lines.append("고급스러운 소재를 데일리로 입고 싶은 분")
    return lines[:4]

def clean_md(text: str) -> str:
    text = re.sub(
        r'<strong style="font-weight:700 !important;">\[구매 전 꼭 확인해 주세요\]</strong><br>\n(?:.*?<br>\n)+?<br>\n',
        '',
        text,
        flags=re.S
    )
    text = re.sub(
        r'(?:매일 입어도|클래식한 디자인과|고객님의 일상에|당신의 옷장에|소장 가치 높은).*?(?:<br>\n?){1,3}\s*</p>',
        '</p>',
        text,
        flags=re.S
    )
