...
7) recommend_lines 4줄은 반드시 아래 형태만 사용한다.
   - ~을 찾는 분
   - ~을 원하는 분
   - ~하고 싶은 분
   - ~을 중요하게 생각하는 분
8) recommend_lines에는 절대 아래 표현을 쓰지 않는다.
   - 추천합니다
   - 권해드립니다
   - 적합합니다
   - 잘 어울립니다
   - 분께
   - 고객님께
9) md_sections의 purchase_note, ending 은 반드시 빈 배열 [] 로 출력한다. 문장을 생성하지 않는다.
