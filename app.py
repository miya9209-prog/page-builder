...
RECOMMEND_RULE = """
[추천 블록 작성 규칙]
recommend_lines는 반드시 아래 형식으로만 작성한다.
- 오피스룩으로 깔끔하게 입고 싶은 분
- 체형을 자연스럽게 커버하고 싶은 분
...
금지:
- 추천합니다 / 권해드립니다 / 적합합니다 / 잘 어울립니다 / 만족하실 거예요
- 고객님께 / 분께 / 여성분께
모든 문장은 반드시 '분'으로 끝나야 한다.
"""
...
- MD원고: [이 상품을 초이스한 이유입니다.] / [원단과 두께 체감에 대하여] / [체형과 핏, 사이즈 선택 가이드] / [이렇게 입는 날이 많아집니다] 까지만 작성
...
def normalize_recommend_line(x: str) -> str:
    s = clean_line(x)
    s = s.replace("고객님께", "")
    s = s.replace("분께", "분")
    s = s.replace("여성분께", "여성 분")
    s = re.sub(r'(추천드립니다|추천합니다|권해드립니다|권합니다|잘 어울립니다|적합합니다|만족하실 거예요|만족하실 만한 선택입니다)$', '', s).strip()
    s = re.sub(r'\s+', ' ', s).strip()
    if "찾" in s and not s.endswith("분"):
        s = s.split("찾",1)[0] + "찾는 분"
    elif "원하" in s and not s.endswith("분"):
        s = s.split("원하",1)[0] + "원하는 분"
    elif "입고 싶은" in s and not s.endswith("분"):
        s = s.split("입고 싶은",1)[0] + "입고 싶은 분"
    elif "유지하고 싶은" in s and not s.endswith("분"):
        s = s.split("유지하고 싶은",1)[0] + "유지하고 싶은 분"
    elif not s.endswith("분"):
        s = re.sub(r'(입니다|가능합니다|거예요)$','',s).strip()
        if not s.endswith("분"):
            s += " 분"
    return s

def render_text_source(structured: Dict[str, Any]) -> str:
    rec_lines = ''.join(['▪ ' + normalize_recommend_line(x) + '<br>\\n' for x in structured['recommend_lines']])
